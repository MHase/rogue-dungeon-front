...
export default class extends Phaser.Scene {
  ...
  addPlayer(data) {
    ...
    this.physics.add.collider( // add collider to detect when other players shot each other
      player.catBullets,
      this.otherPlayers.getChildren().filter(otherPlayer => otherPlayer !== player),
      (hitPlayer, cat) => {
        cat.hit();
      },
      null,
    ...
    this.physics.add.collider( // also collider to detect when we get hit
      player.catBullets,
      this.player,
      (hitPlayer, cat) => {
        cat.hit();
  ...
  animateTurn(data) {
    const movingPlayer = this.getPlayerFromGroup(data.name);
    movingPlayer.direction = data.dir;
